#!/usr/bin/env bash





    #>
    #> The MFC prologue prints a summary of the running job and starts a timer.
    #>

    . "/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/toolchain/util.sh"

    TABLE_FORMAT_LINE="| * %-14s $MAGENTA%-35s$COLOR_RESET * %-14s $MAGENTA%-35s$COLOR_RESET |\\n"
    TABLE_HEADER="+-----------------------------------------------------------------------------------------------------------+ \\n"
    TABLE_FOOTER="+-----------------------------------------------------------------------------------------------------------+ \\n"
    TABLE_TITLE_FORMAT="| %-105s |\\n"
    TABLE_CONTENT=$(cat <<-END
$(printf "$TABLE_FORMAT_LINE" "Start-time"   "$(date +%T)"                    "Start-date" "$(date +%T)")
$(printf "$TABLE_FORMAT_LINE" "Partition"    "N/A"          "Walltime"   "01:00:00")
$(printf "$TABLE_FORMAT_LINE" "Account"      "N/A"          "Nodes"      "1")
$(printf "$TABLE_FORMAT_LINE" "Job Name"     "MFC"                        "Engine"     "interactive")
$(printf "$TABLE_FORMAT_LINE" "QoS"          "N/A" "Binary"     "N/A")
$(printf "$TABLE_FORMAT_LINE" "Queue System" "Interactive"                "Email"      "N/A")
END
)

    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "MFC case # MFC @ /storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/examples/2D_igr/case.py:"
    printf "$TABLE_HEADER"
    printf "$TABLE_CONTENT\\n"
    printf "$TABLE_FOOTER\\n"


    t_start=$(date +%s)


ok ":) Loading modules:\n"
cd "/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR"
. ./mfc.sh load -c p -m c
cd - > /dev/null
echo

export LD_LIBRARY_PATH=/usr/local/pace-apps/spack/packages/linux-rhel9-x86_64_v3/gcc-11.3.1/cuda-12.1.1-ebglvvqo7uhjvhvff2qlsjtjd54louaf/lib64:$LD_LIBRARY_PATH

    
    ok ":) Running$MAGENTA syscheck$COLOR_RESET:\n"

    cd '/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/examples/2D_igr'

    t_syscheck_start=$(python3 -c 'import time; print(time.time())')


        (set -x;  "/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/build/install/8734d45d7a/bin/syscheck")

    
    code=$?

    t_syscheck_stop=$(python3 -c 'import time; print(time.time())')

    if [ $code -eq 22 ]; then
        echo
        error "$YELLOW CASE FILE ERROR$COLOR_RESET > $YELLOW Case file has prohibited conditions as stated above.$COLOR_RESET"
    fi

    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/build/install/8734d45d7a/bin/syscheck$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi



    echo
    
    ok ":) Running$MAGENTA pre_process$COLOR_RESET:\n"

    cd '/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/examples/2D_igr'

    t_pre_process_start=$(python3 -c 'import time; print(time.time())')


        (set -x;  "/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/build/install/d276aab2da/bin/pre_process")

    
    code=$?

    t_pre_process_stop=$(python3 -c 'import time; print(time.time())')

    if [ $code -eq 22 ]; then
        echo
        error "$YELLOW CASE FILE ERROR$COLOR_RESET > $YELLOW Case file has prohibited conditions as stated above.$COLOR_RESET"
    fi

    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/build/install/d276aab2da/bin/pre_process$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi



    echo
    
    ok ":) Running$MAGENTA simulation$COLOR_RESET:\n"

    cd '/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/examples/2D_igr'

    t_simulation_start=$(python3 -c 'import time; print(time.time())')


        (set -x;  "/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/build/install/3b47c8e2e5/bin/simulation")

    
    code=$?

    t_simulation_stop=$(python3 -c 'import time; print(time.time())')

    if [ $code -eq 22 ]; then
        echo
        error "$YELLOW CASE FILE ERROR$COLOR_RESET > $YELLOW Case file has prohibited conditions as stated above.$COLOR_RESET"
    fi

    if [ $code -ne 0 ]; then
        echo
        error ":( $MAGENTA/storage/coda1/p-sbryngelson3/0/sbryngelson3/MFC-IGR/build/install/3b47c8e2e5/bin/simulation$COLOR_RESET failed with exit code $MAGENTA$code$COLOR_RESET."
        echo
        exit 1
    fi



    echo


    #>
    #> The MFC epilogue stops the timer and prints the execution summary. It also
    #> performs some cleanup and housekeeping tasks before exiting.
    #>

    code=$?

    t_stop="$(date +%s)"

    printf "$TABLE_HEADER"
    printf "$TABLE_TITLE_FORMAT" "Finished MFC:"
    printf "$TABLE_FORMAT_LINE"  "Total-time:" "$(expr $t_stop - $t_start)s" "Exit Code:" "$code"
    printf "$TABLE_FORMAT_LINE"  "End-time:"   "$(date +%T)"                 "End-date:"  "$(date +%T)"
    printf "$TABLE_FOOTER"

    exit $code

